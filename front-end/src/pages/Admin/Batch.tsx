...
import {
	useAdminCohorts,
	useCreateCohort,
} from "@/hooks/admin/useAdminCohorts";
...
const issueSchema = z.object({
	...
	studentWallet: z.string().trim().min(1, "Alamat wallet wajib diisi."),
	file: z.custom<File>((file) => file instanceof File, {
		message: "Unggah berkas ijazah.",
	}),
});
...
export default function AdminBatch() {
	...
	const [batchResult, setBatchResult] = useState<BatchIssueResponse | null>(
		null,
	);
	...
	const createCohortMutation = useCreateCohort((cohort) => {
		issueForm.setValue("cohortId", cohort.id, {
			shouldDirty: true,
			shouldValidate: true,
		});
	...
	function parseList(text: string) {
		...
		for (const line of lines.slice(1)) {
			...
			if (!name || !email || !wallet) continue;
			next.push({ name, email, wallet });
		}
		setRows(next);
		return next;
	}
	...
	async function handleListFile(file?: File | null) {
		...
		try {
			...
			if (parsed.length === 0) {
				setBatchError(
					"Daftar belum terbaca. Gunakan format: name,email,wallet",
				);
			}
		} catch {
			setRows([]);
	...
	return (
		<Container className="space-y-8">
			<SectionTitle
				...
				subtitle="Isi data siswa lalu unggah berkas ijazah."
			/>
			...
			<Card className="bg-white/90 backdrop-blur">
				<CardHeader>
					...
					<CardDescription>
						Pilih kelompok yang sudah ada, atau buat baru.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
					<Select<CohortOption, false>
						...
						onChange={(option) => {
							...
							issueForm.setValue("cohortId", id, {
								shouldDirty: true,
								shouldValidate: true,
							});
						}}
					...
					<form className="flex gap-2" onSubmit={submitCreateCohort}>
						<Input
							placeholder="Contoh: XII IPA 2026"
							{...cohortForm.register("label")}
						/>
						<Button
							type="submit"
							variant="outline"
					...
					{cohortsQuery.isLoading && (
						<p className="text-xs text-mutedForeground">Memuat kelompok...</p>
					)}
				</CardContent>
			</Card>
			...
			<Card className="bg-white/90 backdrop-blur">
				<CardHeader>
					...
					<CardDescription>
						Gunakan form ini untuk menerbitkan ijazah satu per satu.
					</CardDescription>
				</CardHeader>
				<CardContent>
					<form className="grid gap-3" onSubmit={submitIssue}>
						...
						<Input
							...
							type="email"
							{...issueForm.register("studentEmail")}
						/>
						<Input
							placeholder="Alamat wallet siswa (0x...)"
							{...issueForm.register("studentWallet")}
						/>
						...
						<div className="flex items-center gap-3">
							<Button
								type="submit"
								disabled={
									!issueForm.formState.isValid || issueMutation.isPending
								}
							>
					...
					{result && (
						<div className="mt-4 rounded-md border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
							...
							{result.temp_password && (
								<p>Kata sandi awal: {result.temp_password}</p>
							)}
						</div>
					)}
				</CardContent>
			...
			<Card className="bg-white/90 backdrop-blur">
				<CardHeader>
					...
					<CardDescription>
						Unggah daftar siswa dan berkas ijazah, lalu terbitkan sekaligus.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="flex flex-wrap items-center gap-3">
						...
						<span className="text-xs text-mutedForeground">
							Isi contoh: name, email, wallet
						</span>
					</div>

					<div className="space-y-2">
						<p className="text-xs text-mutedForeground">
							Unggah daftar siswa (CSV)
						</p>
						<Input
							type="file"
							accept="text/csv"
							onChange={(e) => void handleListFile(e.target.files?.[0])}
						/>
						{listFileName && (
							<p className="text-xs text-mutedForeground">
								Daftar: {listFileName}
							...
							<ul className="mt-2 space-y-1">
								{failedItems.slice(0, 8).map((item) => (
									<li key={`${item.row}-${item.student_email}`}>
										Baris {item.row} - {item.student_name || "-"} (
										{item.student_email || "-"}) : {item.error}
									</li>
								))}
							</ul>
						</div>
