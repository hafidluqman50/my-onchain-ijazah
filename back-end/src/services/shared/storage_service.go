...
func NewStorageService(provider string, localDir string, pinataJWT string, pinataGateway string) (*StorageService, error) {
	...
	if provider == StorageProviderPinata && strings.Count(strings.TrimSpace(pinataJWT), ".") != 2 {
		return nil, fmt.Errorf("pinata jwt appears malformed; expected 3 JWT segments")
	}
	return service, nil
}
